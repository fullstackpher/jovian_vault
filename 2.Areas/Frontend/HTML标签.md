...
# p标签
`<p>`标签是一个块级元素，通常用于表示段落。它独占一行，并且上下会有一些默认的边距
- **块级元素**：默认独占一行
- **主要用途**：表示文本段落
- **默认样式**：有上下边距（margin）
...
# span标签
`<span>`标签是一个内联元素，通常用于对文本的一部分进行样式设置或脚本操作，它不会独占一行，只占据其内容所需的宽度
- **行内元素**：不换行，只占据内容宽度
- **主要用途**：对文本的一部分应用样式
- **默认样式**：无特殊格式
...
示例：
```html
<p>这是一段文本，<span style="color: red;">这部分是红色的</span>，其他正常。</p>
```

## 场景使用场景
- `<p>`：文章段落，内容块，大段文字
- `<span>`：图标文字，特殊样式，动态内容，小段文本标记
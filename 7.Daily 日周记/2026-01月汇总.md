...
// 调试信息
dv.paragraph("---");
dv.paragraph("**各笔记任务分布（共" + dailyNotes.length + "篇笔记）**");
const tableData = taskDetails.map(t => [t.name, t.total, t.completed, t.uncompleted]);
dv.table(["笔记", "总任务", "已完成", "未完成"], tableData);
```
...
for (let i = 12; i <= 31; i++) {
  ...
  const file = app.vault.getFileByPath("7.Daily 日周记/" + dateStr + ".md");
  if (!file) continue;
